...
def convert_bag_to_pcd(bag_file, output_dir):
    """
    Function to convert PointCloud2 data from a ROS bag file to individual PCD files for each frame.
    ...
    try:
        ...
        for _, msg, _ in bag.read_messages(topics=[pointcloud2_topic]):
            ...
            output_file = os.path.join(output_dir, f"{base_filename}_frame_{message_count:04d}.pcd")
            print("Creating PCD file:", output_file)

            # Create a point cloud object
            points = np.vstack((x_list, y_list, z_list)).transpose()
...
if __name__ == "__main__":
    ...
    output_dir = "/media/livox-tele/ExtremeSSD/people_moving_pcd"
    ...
    for bag_file in os.listdir(bag_directory):
        if bag_file.endswith(".bag"):
            ...
            convert_bag_to_pcd(bag_file_path, output_dir)
